...
// Inicia um chat novo: apaga qualquer chat/worktree anterior do board, cria a
// worktree na branch selecionada e roda o primeiro turno dentro dela.
export async function startChat({ boardId, repo, branch, model, effort, prompt }) {
  ...
  db().prepare(`
    INSERT INTO board_chats (board_id, session_id, repo, branch, model, effort, worktree_path, thread, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(boardId, sessionId, repo, branch, model || "sonnet", effort || "medium", worktreeDir, JSON.stringify(thread), now, now);
...
export async function sendMessage({ boardId, prompt, model, effort }) {
  ...
  thread.push({ role: "user", text: prompt.trim() }, { role: "assistant", text });
  db().prepare(`
    UPDATE board_chats SET thread = ?, model = ?, effort = ?, updated_at = ? WHERE board_id = ?
  `).run(JSON.stringify(thread), model || row.model, effort || row.effort, new Date().toISOString(), boardId);
